...
| the dispenser pile's broom verb (`broomed`) | `EX_LocalVirtualFunction` from the broom's own ubergraph | invisible to a ProcessEvent hook | the script-body gate refuses it per call on a client and forwards the pile's save key as a `BroomIntent`; the host runs the game's own verb |
| the broom's own chip-pile morph (a copy of the pile's morph inside the broom's ubergraph) | none: a `BeginDeferredActorSpawnFromClass` / `FinishSpawningActor` / `K2_DestroyActor` sequence in the broom's graph, no verb of its own | the finish and the destroy reach their native seams, but the pile-to-clump birth seam declines it, because the spawn is issued by the broom's bytecode and the seam recognises a birth only from a pile's | **OPEN**: nothing refuses it on a client and nothing expresses it on the host, so sweeping chip piles does not cross `[V]` |
