...
the budget keeps refilling, and the arithmetic says the whole character set fits the atlas at
about 86% of it, with a detector watching for the packing failure that arithmetic would miss
(`ui/atlas_watch`). While any interactive surface is up the overlay
