...
// The clump-mirror test (harness/autotest/autotest_clump.cpp): the host spawns a
// prop_garbageClump_C, writes it to grabbing_actor so the held-prop send broadcasts it, then sweeps
// it; the client mirrors it kinematically. The non-Aprop_C kinematic path, and no crash. Env
...
// The clump visibility probe (harness/autotest/autotest_clump.cpp), solo: spawns a bare
...
// The flashlight-toggle test (harness/autotest/autotest_flashlight.cpp): the local flashlight is
...
// The config-corpus selftest (harness/autotest/autotest_config.cpp), solo and role-agnostic, no
...
// The weather sync test (harness/autotest/autotest_weather.cpp), host: after the settle,
// DebugForceRain forces rain on, off, on, off at 5 s intervals; the observer on setRainProperties
// broadcasts each. Pass: two or more host broadcast lines, two or more client applied lines, and
// both peers' isRaining diagnostic matching after each toggle. Blocks about 30 s. Env
...
// The re-seed completeness probe (harness/autotest/autotest_tracker_selftest.cpp), both peers:
// after a 25 s settle (past the boot-time level travel) ReSeedKnownKeyedProps runs on the game
// thread and logs how many new live keyed props it adds; a large count means the boot seed ran on
// the pre-travel world and the story map's placed props were untracked. Env
// VOTVCOOP_RUN_RESEED_TEST=1.
void RunAutonomousReSeedTest();
DWORD WINAPI ReSeedTestThread(LPVOID arg);

// The ragdoll wire test (harness/autotest/autotest_ragdoll.cpp), both peers: the client drives
...
// The puppet-frame shot (harness/autotest/autotest_puppetframe.cpp): the host stands about 420
// units back from the standing slot-1 puppet, aims at its head and holds the frame so mp.py
// puppetshot can capture the nameplate; the client stands. Env VOTVCOOP_RUN_PUPPET_FRAME=1.
void RunAutonomousPuppetFrame();
DWORD WINAPI PuppetFrameThread(LPVOID arg);

// The hurt-flash wire test (harness/autotest/autotest_damage.cpp), both peers: the client lowers
// its own saveSlot.health in steps, the vitals stream carries the fraction, and the host confirms
// its slot-1 puppet's nameplate flashes red (RemotePlayer::IsHurtFlashing), with no new wire. Env
...
// The damage relay test (harness/autotest/autotest_playerdmg.cpp), both peers: the host sends a
// synthetic PlayerDamage to slot 1 (player_damage::DebugForceHitPuppet), the client applies Add
// Player Damage to its own player, and the client's streamed health drop flashes the host's slot-1
...
// The native death-chain instrument (harness/autotest/autotest_death.cpp), solo and sessionless (no
// net role, so the local-death handling never fires and VOTV's own death plays out): a lethal Add
...
// The ragdoll spawn probe (harness/autotest/autotest_ragdoll_spawn_probe.cpp), single instance, no
...
// The menu-travel probe (harness/autotest/autotest_menutravel_probe.cpp), single instance, no
// connection: settles in gameplay and tries the candidate travel commands in series
...
// The re-load churn probe (harness/autotest/autotest_reloadchurn.cpp), a negative control: solo and
...
// The zero-AV drill for the CachedObjRef discipline (harness/autotest/autotest_islive_drill.cpp): a
// fake object on a decommitted page; a bare IsLive must fault exactly once (absorbed by its SEH)
// and CachedObjRef::Alive must answer false with no AV. A scoped diagnostic VEH covers the drill
// body only. Env VOTVCOOP_RUN_ISLIVE_DRILL=1; the dev smoke lane only.
DWORD WINAPI IsLiveDrillThread(LPVOID arg);

// The fog probe (harness/autotest/autotest_fog_probe.cpp), single instance, no connection: forces
// fog on through the cycle's own spawnFog and superFogEvent, samples finalFogDensity, thickFog,
...
// The HUD red-tint discriminator (harness/autotest/autotest_hud_tint.cpp), solo:
// ui_damageIndicator_C (a child of ui_UI_C, invisible to a viewport-widget census) holds two
// candidates for a red wash, dmg_tunnel, which has no Visibility property and draws mat_tunnel
// full-screen every frame, and dmg_full, the death branch's one-way latch; a cooked material's
// transparency at alpha 0 cannot be read statically. Four arms (baseline, the whole indicator
// collapsed, dmg_tunnel only, dmg_full only), one screenshot marker each, everything restored on
// exit; if the red survives arm B the indicator is excluded. Env VOTVCOOP_RUN_HUD_TINT_PROBE=1;
// mp.py hudtint.
DWORD WINAPI HudTintProbeThread(LPVOID arg);

// The movement oscillator (harness/autotest/autotest_move_osc.cpp), role-agnostic: circles the
// local player so the other peer's RemotePlayer interpolation has a moving source (with a static
...
// the POST snapshot reading shots 0. Greppable "eventforce_test: VERDICT". Env
...
// both edges and broadcast, and the client's own "applied active=N" lines are the assert. Env
// VOTVCOOP_RUN_ALARMFORCE_TEST=1.
void RunAutonomousAlarmForceTest();
DWORD WINAPI AlarmForceTestThread(LPVOID arg);

// The wisp mirror-lane smoke (harness/autotest/autotest_wisplane.cpp), host: ForceNow the wisps
// event (the swarm spawns ~32 wisp_C through EX_CallMath, which the Func-thunk catch must enrol and
...
// The killerwisp acquisition probe (harness/autotest/autotest_kwisp_probe.cpp), host, read-only:
...
// The pause-guard test (harness/autotest/autotest_pauseguard.cpp), client: after the settle the
// world is paused through the game's own SetGamePaused (the state the ESC menu engages) and
// IsGamePaused sampled; pass if the pause reads false within 1 s and the log shows the guard's
// un-pause line. Env VOTVCOOP_RUN_PAUSE_TEST=1.
void RunAutonomousPauseGuardTest();
DWORD WINAPI PauseGuardTestThread(LPVOID arg);

// The piramid mirror-lane smoke (harness/autotest/autotest_piramidforce.cpp), host:
// ForceNow("piramid") runs the real native chain (the trigger overlap, the spawner's runTrigger,
// four killerwisp_C and a piramid2_C at scale 2); the lane must arm (DebugHooksArmed), and the
// wisps are re-pinned onto a 150 m ring around the pyramid every 5 s until its own brain acquires,
// arrives and gathers, DebugHostRelayCount >= 1 being the pass. The client side is a log diff. Env
// VOTVCOOP_RUN_PIRAMIDFORCE_TEST=1.
void RunAutonomousPiramidForceTest();
DWORD WINAPI PiramidForceTestThread(LPVOID arg);

// The bot-director halt probe (harness/autotest/autotest_navprobe.cpp), solo: two gates measured
// before any director is built. Gate A: UNavigationSystemV1::FindPathToLocationSynchronously
// returns a traversable path over VOTV's baked NavMesh to a reachable endpoint (call failed, no
// route, or traversable). Gate B: a reflected APawn::AddMovementInput (resolved on Pawn, not the
// leaf class) moves the possessed body, swept over four directions, horizontal displacement only,
